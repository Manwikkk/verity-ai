...
async function main() {
    ...
    try {
        await prisma.$executeRawUnsafe(`ALTER TABLE document_chunks ADD COLUMN IF NOT EXISTS embedding vector(384)`);
    ...
    try {
        await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_chunks_embedding ON document_chunks USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)`);
        console.log("  ✓ vector index created");
    }
    catch (err) {
        // IVFFlat needs enough rows — use HNSW instead for small datasets
        try {
            await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_chunks_embedding ON document_chunks USING hnsw (embedding vector_cosine_ops)`);
    ...
    try {
        await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_chunks_fts ON document_chunks USING gin (to_tsvector('english', content))`);
    ...
    const sampleDocs = [
        { name: "HR Handbook v4.2.pdf", mimeType: "application/pdf", sizeBytes: 2516582, pageCount: 184, owner: "People Ops", status: "READY", chunkCount: 412 },
        { name: "Vendor Master Service Agreement — Template.docx", mimeType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document", sizeBytes: 190464, pageCount: 22, owner: "Legal", status: "READY", chunkCount: 87 },
        { name: "Finance Operations Manual 2026.pdf", mimeType: "application/pdf", sizeBytes: 5347737, pageCount: 312, owner: "Finance", status: "READY", chunkCount: 1284 },
        { name: "GDPR Processor Obligations.pdf", mimeType: "application/pdf", sizeBytes: 421888, pageCount: 36, owner: "Compliance", status: "READY", chunkCount: 142 },
        { name: "Q2 Procurement SOPs.pdf", mimeType: "application/pdf", sizeBytes: 1258291, pageCount: 64, owner: "Procurement", status: "CHUNKING", chunkCount: 0 },
    ];
